...
public class SecurityConfig implements WebMvcConfigurer {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.csrf().disable().exceptionHandling().authenticationEntryPoint(authEntryPoint).and().sessionManagement()
				...
				.antMatchers("/api/insert/**").permitAll().antMatchers("/api/auth/**").permitAll()
				.antMatchers("/api/admin**").hasAuthority("ADMINISTRADOR").antMatchers("/api/member**")
				.hasAuthority("MEMBRO").anyRequest().authenticated().and().httpBasic();
